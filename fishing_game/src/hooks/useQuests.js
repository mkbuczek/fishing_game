...
export function useQuests(initialSavedState) {
    ...
    function incrementScopedCatch(source) {
        setQuestState((prev) => {
            const next = { ...prev };
            ...
            questNpcs.forEach((npc) => {
                ...
                const updatedObjectives = npcState.activeQuest.objectives.map((objective) => {
                    ...
                    return { ...objective, progress: objective.progress + 1 };
                });
    ...
    function turnInFish(npcId, objectiveId, instanceIds, setInventory) {
    ...
    function claimQuest(npcId, setPearls) {
